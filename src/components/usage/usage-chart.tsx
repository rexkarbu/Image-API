...
export function UsageChart({ timeSeries, totalUnits }: UsageChartProps) {
  ...
  // Decide how many x-axis labels to display based on bucket count
      ...
      <div className="rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-6 shadow-sm space-y-4">
        ...
        <div className="h-44 rounded-md border border-dashed border-neutral-200 dark:border-neutral-800 flex flex-col items-center justify-center p-6 text-center space-y-1">
          ...
          <p className="text-[11px] text-neutral-400 dark:text-neutral-500">
            Incoming requests to <code className="bg-neutral-100 dark:bg-neutral-800 px-1 py-0.5 rounded">POST /v1/images/transform</code> will appear here in real-time.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-6 shadow-sm space-y-4">
      ...
      <div className="relative pt-4 pb-2" aria-hidden="true">
        <div className="flex items-end space-x-1 sm:space-x-1.5 h-40 w-full">
              ...
              >
                ...
                <div className="absolute bottom-full mb-1 hidden group-hover:flex group-focus:flex flex-col items-center pointer-events-none z-10">
                  <div className="bg-neutral-900 dark:bg-neutral-100 text-white dark:text-neutral-900 text-[10px] rounded px-2 py-1 shadow whitespace-nowrap">
                    <div className="font-semibold font-mono">{bucket.units.toLocaleString()} {bucket.units === 1 ? "unit" : "units"}</div>
                    <div className="text-[9px] text-neutral-400 dark:text-neutral-600">{bucket.label} (UTC)</div>
                  </div>
                </div>
        ...
        <div className="flex justify-between items-center text-[10px] font-mono text-neutral-400 dark:text-neutral-500 pt-2 border-t border-neutral-100 dark:border-neutral-800">
          ...
          <span>{timeSeries[timeSeries.length - 1]?.label || "Now"} (UTC)</span>
        </div>
      </div>

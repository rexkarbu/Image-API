...
export function UsageKeyBreakdown({ keyBreakdown, totalUnits }: UsageKeyBreakdownProps) {
  if (keyBreakdown.length === 0) {
    return (
      <div className="rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-5 shadow-sm space-y-3">
    ...
    <div className="rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-5 shadow-sm space-y-3">
      ...
      <div className="space-y-2.5">
        {keyBreakdown.map((key) => {
          const hasUsage = totalUnits > 0 && key.units > 0 && key.percentage !== null;
          const percentDisplay = hasUsage
          ...
          return (
            <div
              key={key.apiKeyId}
              className="p-3 rounded-lg border border-neutral-100 dark:border-neutral-800/80 bg-neutral-50/50 dark:bg-neutral-900/40 space-y-2"
            >
              ...
              {/* Progress bar: Render fill only when totalUnits > 0 and key has usage */}
              <div className="w-full h-1.5 rounded-full bg-neutral-200 dark:bg-neutral-800 overflow-hidden">
                {hasUsage && (
                  <div
                    className="h-full rounded-full bg-neutral-900 dark:bg-neutral-100 motion-safe:transition-all motion-safe:duration-300"
                    style={{
                      width: `${Math.min(Math.max(key.percentage!, 2), 100)}%`,
                    }}
                  />
                )}
              </div>

...
- **Runbook**: [Incident Response Runbook - Section 1](file:///d:/project/hlf/docs/runbooks/incident-response.md#1-readiness-failure-p1)

### 2. High API Error Rate (`ALERT_API_5XX_ELEVATED`)
- **Severity**: `P1 - Critical`
- **Signal**: `POST /v1/images/transform` 5xx responses exceed 1% of total requests over a 5-minute rolling window.
...
- **Runbook**: [Incident Response Runbook - Section 2](file:///d:/project/hlf/docs/runbooks/incident-response.md#2-elevated-5xx-rate-p1)
...
- **Runbook**: [Incident Response Runbook - Section 3](file:///d:/project/hlf/docs/runbooks/incident-response.md#3-rate-limiter-outage-p1)

### 4. High Image Transformation Latency (`ALERT_LATENCY_P95_ELEVATED`)
- **Severity**: `P2 - High`
- **Signal**: `POST /v1/images/transform` P95 latency > 250ms over a 15-minute rolling window.
- **Impact**: Slow API performance for developers.
- **Root Causes**: Oversized source images, CPU contention, database connection latency.
- **Runbook**: [Incident Response Runbook - Section 4](file:///d:/project/hlf/docs/runbooks/incident-response.md#4-high-transformation-latency-p2)
...
- **Runbook**: [Incident Response Runbook - Section 5](file:///d:/project/hlf/docs/runbooks/incident-response.md#5-webhook-inbox-backlog-p2)
...
- **Runbook**: [Incident Response Runbook - Section 6](file:///d:/project/hlf/docs/runbooks/incident-response.md#6-meter-event-reporting-failure-p2)
...
- **Runbook**: [Incident Response Runbook - Section 7](file:///d:/project/hlf/docs/runbooks/incident-response.md#7-reconciliation-mismatch-p3)

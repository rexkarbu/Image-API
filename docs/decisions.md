...
17. **Rate-Limit Pure-Core Separation, Strict Vercel Trust, and Raw Secret Invariants (Milestone 4.2)**
    - Pure Core vs. Server-Only Architecture: Extracted pure validation, normalization, HMAC derivation, and Redis safety guards into standalone modules (`src/lib/security/rate-limit-core.ts`, `src/lib/ratelimit/redis-safety-core.ts`) without `server-only` imports. Server runtime wrappers (`client-ip.ts`, `rate-limit-identifiers.ts`, `redis-safety.ts`, `limiter.ts`) retain `import "server-only";` while CLI / test scripts safely utilize identical canonical algorithms.
    - Exact Vercel Trust Verification: Production environments trust `x-vercel-forwarded-for` strictly when `process.env.VERCEL === "1"`. Presence of `VERCEL_ENV` alone never authorizes proxy header trust.
...
19. **Modern Stripe Billing Meters, Webhooks, Usage Reporting & Reconciliation (Milestone 5)**
    - Modern Stripe Billing Meters: Replaced deprecated legacy Usage Records with modern Stripe Billing Meters (`stripe.billing.meterEvents.create` and `stripe.billing.meters.listEventSummaries`).
    - Immutable Local Source of Truth: `usage_events` remains the immutable local billing source of truth. The transformation hot path (`POST /v1/images/transform`) never calls, queries, or waits on Stripe.
    - Database-Backed Worker Leasing & Atomic Batch Claiming: Background billing worker acquires a distributed lease (`billing_worker_leases`), claims closed usage windows into `billing_usage_batches`, and maps `billing_usage_batch_items` with a unique constraint on `usage_event_id` preventing duplicate reporting across concurrent workers.
    - Durable Webhook Inbox & Event Ordering: Webhooks verify raw signatures and insert minimal metadata into `billing_webhook_events`. Event processing utilizes provider event timestamps and object IDs to prevent stale out-of-order state overwrites without storing sensitive payment details or raw payloads.
    - Two-Layer Usage Reconciliation: Reconciles local eligible units, batched items, reported batches, and provider-side Stripe Meter Event Summaries, accounting for asynchronous provider settlement delay before asserting mismatch.
    - Owner-Only Authorization & Safe Onboarding: Customer provisioning is enqueued atomically during organization creation and executed after commit without blocking onboarding. Hosted Checkout and Customer Portal mutations are strictly restricted to organization owners with server-derived context.
    - Bounded Production Scheduling: Dedicated `/api/cron/billing` endpoint and CLI script `pnpm billing:worker` implement bounded processing. Automated scheduler activation is deferred to Milestone 6.

20. **Observability, OpenTelemetry Tracing, Health Probes, OpenAPI Spec & Deployment Preflight (Milestone 6.0)**
    - Structured JSON Request Logging: Emits single-line JSON log entries with strict field allowlisting (`timestamp`, `level`, `event`, `service`, `environment`, `requestId`, `traceId`, `spanId`, `route`, `method`, `statusCode`, `durationMs`, `outcome`, `errorCode`, `details`). Recursive redaction automatically filters secrets, passwords, connection strings, and credential prefixes (`img_live_`, `sk_live_`, `sk_test_`, `whsec_`).
    - Bounded Request-ID Contract: Inbound `X-Request-ID` is validated against `^[A-Za-z0-9._:-]{1,128}$` or securely generated as a UUID, and returned on every response for diagnostic correlation.
    - Low-Cardinality OpenTelemetry Spans: Initializes Next.js instrumentation under service `image-api` (`src/instrumentation.ts` and `withSpan()`), recording low-cardinality attributes across critical request lifecycles without attaching sensitive PII, bodies, or tokens.
    - Dual Health Check Endpoints: `GET /api/health/live` verifies process liveness with zero external network I/O; `GET /api/health/ready` verifies critical request-path dependencies (PostgreSQL read-check + Redis ping) in parallel with a 2500ms timeout. Stripe is excluded from the readiness blocking path.
    - Canonical OpenAPI 3.1.1 Specification & Interactive Portal: Canonical spec exposed via `GET /openapi.json` and interactive Swagger UI at `/docs` with `persistAuthorization: false` and test credential warnings. Internal webhooks (`/api/webhooks/stripe`) and billing crons (`/api/cron/billing`) are strictly excluded from the public OpenAPI document.
    - Deployment Preflight & Explicit Migration Policy: `pnpm deploy:preflight` and `pnpm deploy:verify` validate environment configuration and deployment targets. Database migrations (`pnpm db:migrate`) remain a separate explicit protected step and are never executed during `next build`.
